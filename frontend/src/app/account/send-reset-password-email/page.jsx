...
const ResetPasswordLink = () => {
  ...
  const { handleResetPasswordLink } = useAuth();
  ...
  const { values, errors, handleChange, handleSubmit } = useFormik({
    ...
    onSubmit: async (values) => {
      setLoading(true);
      const data = await handleResetPasswordLink(values.email);
      setLoading(false);
      if (data.status === "success") {
        setServerSuccessMessage(data.message);
      } else {
        setServerErrorMessage(data.message || "Failed to send reset password link");
      }
    },
  });
    ...
    <div className="flex items-center justify-center h-screen bg-gray-100">
      <div className="w-full max-w-md p-8 bg-white rounded-lg shadow-lg">
        ...
        {serverSuccessMessage && (
          <div className="text-sm text-green-500 font-semibold px-2 text-center">{serverSuccessMessage}</div>
        )}
        {serverErrorMessage && (
          <div className="text-sm text-red-500 font-semibold px-2 text-center">{serverErrorMessage}</div>
        )}
      </div>
    </div>
  );
